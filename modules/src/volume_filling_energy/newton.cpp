...
volume_filling_energy_newton(const std::vector<Vector3> &nodes,
  ...
  {
    // add the laplacian
    // this is also the hessian of the curve length energy
    ...
    // add the hessian of the medial axis energy
    for (int i = 0; i < active2MedialAxisPairs.size(); i++) {
      int vx = active2MedialAxisPairs[i].first;
      int vy = active2MedialAxisPairs[i].second;

      std::array<int, 2> v = {vx, vy};
      
      Vector3 medialAxis = active2MedialAxisPositions[i];
      Vector3 bx = (medialAxis - nodes[vx]).normalize();
      Vector3 by = (medialAxis - nodes[vy]).normalize();

      Vector3 db = bx - by;

      Vector3 dRdx = - db / (db.norm() * db.norm());
      Vector3 dRdy = db / (db.norm() * db.norm());
      ...
      for (int j = 0; j < 2; j++) {
        for (int k = 0; k < 2; k++) {
          // for each (j,k) block...
          const Vector3& b    = (j == 0 ? bx : by);    // <-- row‐block chooses which b
          const Vector3& dR   = (k == 0 ? dRdx : dRdy); // <-- column‐block chooses which ∇R
          int              v0 = v[j];
          int              v1 = v[k];

          double norm2 = db.norm() * db.norm();
          Vector3 dRdx = -db / norm2;
          Vector3 dRdy =  db / norm2;

          for (int p = 0; p < 3; ++p) {
            for (int q = 0; q < 3; ++q) {
              double Hij = -options.alpha * b[p] * dR[q];
              int row = node2ActiveNodeIdx[v0]*3 + p;
              int col = node2ActiveNodeIdx[v1]*3 + q;
              triplets.emplace_back(row, col, Hij);
              H(j*3 + p, k*3 + q) = Hij;
            }
          }
        }
      }
      // std::cout << "vx: " << nodes[vx] << ", vy: " << nodes[vy] << std::endl;
  ...
  func.x_to_data(d, [&] (int v_idx, const Eigen::Vector3d& p) {
    ...
    descent[idx] = Vector3 {p(0), p(1), p(2)} / 2;
  });
  ...
  {
    // std::ofstream ofs("./newton-preconditioner.txt");
    // for (int i = 0; i < L.rows(); i++) {
    //   for (int j = 0; j < L.cols(); j++) {
    //     if (L.coeff(i, j) != 0) {
    //       ofs << i << " " << j << " " << L.coeff(i, j) << std::endl;
    //     }
    //   }
    // }
  }
